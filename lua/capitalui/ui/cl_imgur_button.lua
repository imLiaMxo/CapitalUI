...
function PANEL:Init()
    self.ImageCol = Capital.UI.CopyColor(color_white)
...
function PANEL:Paint(w, h)
    ...
    if not self:IsEnabled() then
        Capital.UI.DrawImgur(imageOffset, imageOffset, imageSize, imageSize, self:GetImgurID(), self:GetDisabledColor())
        return
    end
    ...
    self.ImageCol = Capital.UI.LerpColor(FrameTime() * 12, self.ImageCol, col)

    Capital.UI.DrawImgur(imageOffset, imageOffset, imageSize, imageSize, self:GetImgurID(), self.ImageCol)
end

vgui.Register("Capital.UI.ImgurButton", PANEL, "Capital.UI.Button")